...
export const agentLabels: Record<AgentStatus, string> = {
  ...
  running: 'Actualizando el Brief…',
  failed: 'El análisis necesita un reintento',
  agent_configuration_required: 'Falta configurar la IA',
};
export const POLL_INTERVAL = 10_000;
export const POLL_MAX_DURATION = 30 * 60_000;
export function shouldPoll(status: AgentStatus, errors: number, elapsed: number) {
  return (
    (status === 'pending' || status === 'running') && errors < 5 && elapsed < POLL_MAX_DURATION
  );
}
...
export function documentText(node: TiptapNode): string {
  if (node.type === 'text') return node.text ?? '';
  const separator = ['doc', 'bulletList', 'orderedList'].includes(node.type) ? '\n' : '';
