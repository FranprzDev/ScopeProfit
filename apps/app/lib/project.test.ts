...
test('polling only operates while pending and stops at time/error limits', () => {
  assert.equal(shouldPoll('pending', 0, 0), true);
  assert.equal(shouldPoll('running', 4, 100), true);
  for (const status of ['idle', 'failed', 'agent_configuration_required'] as const)
    assert.equal(shouldPoll(status, 0, 0), false);
  assert.equal(shouldPoll('running', 5, 0), false);
  assert.equal(shouldPoll('running', 0, POLL_MAX_DURATION), false);
});
test('Tiptap plain text roundtrip preserves lines and treats markup as text', () => {
  const text = 'Primera línea\n<script>alert(1)</script>\n';
  assert.equal(documentText(textDocument(text)), text);
});
test('image limits reject unsupported files and oversized uploads', () => {
...
test('API forwards project access and surfaces structured failures', async () => {
  ...
  try {
    globalThis.fetch = async (_input, init) => {
      assert.equal(new Headers(init?.headers).get('x-project-token'), 'project-token');
