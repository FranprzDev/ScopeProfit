...
export function fail(code: string, status = 400, message = code): never {
...
export function encryptionKey() {
  ...
  if (key.length !== 32) throw new Error('ENCRYPTION_KEY must encode 32 bytes');
  return key;
}
...
export const authorizedTelegramIds = () =>
