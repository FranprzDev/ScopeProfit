...
export class AgentService implements OnModuleInit,OnModuleDestroy {
 ...
 const gate=await evaluate({model:'typesafe-ai/jev',state:{brief:project.brief?.data,messages:project.messages.map(m=>({role:m.authorRole,content:m.content}))},questions:{needsMoreInfo:{type:'boolean',instructions:'¿El brief todavía necesita información del cliente antes de poder estimar el alcance?'}}});
