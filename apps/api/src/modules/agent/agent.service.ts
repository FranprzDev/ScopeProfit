...
export class AgentService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit() {
    this.saver = PostgresSaver.fromConnString(process.env.DATABASE_URL!);
    await this.saver.setup();
    this.graph = new StateGraph(AgentState)
      .addNode('diagnose', async (state) => ({
        brief: await this.diagnose(state.projectId),
      }))
  ...
  async tick() {
    ...
    try {
      await this.db.project.updateMany({
        where: {
          agentStatus: 'running',
          agentStartedAt: { lt: new Date(Date.now() - 300000) },
        },
  ...
  async process(id: string) {
    ...
    try {
      const project = await this.db.project.findUniqueOrThrow({
        where: { id },
        include: { owner: true, brief: true },
      });
      if (!project.owner.encryptedApiKey) {
      ...
      const saved = await this.db.$transaction(async (tx) => {
        ...
        await tx.message.create({
          data: {
            ...
            threadId: id,
            authorRole: 'agent',
            content: brief.questions.length
              ? `${brief.summary}\n\n${brief.questions
                  .slice(0, 3)
                  .map((q) => q.question)
                  .join('\n')}`
      ...
      const newer = await this.db.message.findFirst({
        where: {
          projectId: id,
          authorRole: { not: 'agent' },
          ...(lastMessage ? { createdAt: { gt: lastMessage.createdAt } } : {}),
        },
      });
      await this.db.project.update({
        where: { id },
        data: {
          agentStatus: !saved || newer ? 'pending' : 'idle',
          agentError: null,
        },
      });
      console.log(
  ...
  async diagnose(id: string): Promise<BriefData> {
    const project = await this.db.project.findUniqueOrThrow({
      where: { id },
      include: {
        owner: true,
    ...
    const model = new ChatGoogleGenerativeAI({
      apiKey: decrypt(project.owner.encryptedApiKey!),
    ...
    const content: any[] = [
      {
        ...
        text: JSON.stringify({
          ...
          manualDocument: project.document?.versions[0]?.editorContent,
        }),
      },
    ];
    for (const file of project.files.slice(-10)) {
      ...
      if (file.mimeType.startsWith('image/'))
        content.push({
          type: AgentContentType.ImageUrl,
          image_url: `data:${file.mimeType};base64,${data}`,
        });
      else
        content.push({
    ...
    const output = await model
      .withStructuredOutput(briefSchema)
      .invoke(
        [
          new SystemMessage(
