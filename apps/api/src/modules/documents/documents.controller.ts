...
class SaveEditorDto {
  ...
  @IsObject() editorContent!: unknown;
}
...
export class DocumentsController {
  ...
  @Get() async list(
    ...
    const items = await this.db.documentVersion.findMany({
      where: { documentId: document.id },
      orderBy: { version: 'desc' },
      take: n + 1,
      ...(cursor
        ? {
            cursor: { documentId_version: { documentId: document.id, version: Number(cursor) } },
            skip: 1,
          }
        : {}),
    });
    ...
    if (more) items.pop();
    return ok({
      items: items.map((v) => this.view(v)),
      nextCursor: more ? String(items.at(-1)!.version) : null,
    });
  }

  @Patch('current') async saveEditor(
    @Req() req: Request,
    @Param('id') id: string,
    @Body() body: SaveEditorDto,
  ) {
    const user = await this.user(req);
    await this.auth.project(user, id, this.token(req));
    return ok(
  ...
  @Get(':version/:format') async download(
    @Req() req: Request,
    @Param('id') id: string,
    @Param('version') version: string,
    @Param('format') format: 'md' | 'pdf' | 'docx',
    @Res() res: Response,
  ) {
    const user = await this.user(req);
    await this.auth.project(user, id, this.token(req));
  ...
  private view(v: any) {
    return {
      id: v.id,
      version: v.version,
      briefVersion: v.briefVersion,
      status: v.status,
      editorContent: v.editorContent ?? null,
      createdAt: v.createdAt.toISOString(),
      formats: Object.keys(v.artifacts ?? {}),
    };
  }
}
