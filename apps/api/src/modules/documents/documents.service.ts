...
export class DocumentsService {
  ...
  async generate(
    ...
    if (expectedVersion !== undefined && expectedVersion !== current)
      return fail('VERSION_CONFLICT', 'Document changed; reload before saving');
    ...
    for (const format of ['md', 'pdf', 'docx'] as const)
      artifacts[format] = await this.storage.write(
        `${projectId}/documents/${version}/${generation}.${format}`,
        buffers[format],
      );
    return this.prisma.$transaction(
      async (tx) => {
        ...
        const updated = await tx.document.updateMany({
          where: { id: document.id, currentVersion: current },
          data: { currentVersion: version },
        });
        if (updated.count !== 1)
          return fail('VERSION_CONFLICT', 'Document changed during generation');
        const saved = await tx.documentVersion.create({
          data: {
  ...
  async download(projectId: string, version: number, format: 'md' | 'pdf' | 'docx') {
    if (!['md', 'pdf', 'docx'].includes(format))
      return fail('INVALID_FORMAT', 'Unknown document format', 400);
    ...
    return {
      buffer: await this.storage.read(artifact.path),
      name: `scope-${projectId}-v${version}.${format}`,
      mimeType: {
    ...
    await this.prisma.$transaction(
      async (tx) => {
        ...
        if (user?.role !== 'professional')
          return fail('FORBIDDEN', 'Professional access required', 403);
        const project = await tx.project.findUnique({
          where: { id: projectId },
          include: { brief: true, document: true },
        });
        if (!project?.document) return fail('DOCUMENT_NOT_FOUND', 'Document not found', 404);
        if (project.document.currentVersion !== version)
          return fail('VERSION_CONFLICT', 'Approve the current document version');
        const saved = await tx.documentVersion.findUnique({
          where: { documentId_version: { documentId: project.document.id, version } },
        });
        if (!saved || saved.briefVersion !== project.brief?.version)
          return fail('VERSION_CONFLICT', 'Document does not match current brief');
        if (saved.approvedAt) return;
        if (
          !['in_review', 'changes_requested'].includes(project.status) ||
          ['pending', 'running'].includes(project.agentStatus)
        )
          return fail('PROJECT_NOT_REVIEWABLE', 'Project is not ready for review');
        ...
        if (
          ...
          !brief.estimates.length ||
          brief.requirements.some((r) => !r.source.trim()) ||
          brief.estimates.some(
            (e) =>
              !Number.isFinite(e.minHours) ||
              !Number.isFinite(e.maxHours) ||
              e.minHours < 0 ||
              e.maxHours < e.minHours,
          )
        )
          return fail(
            'DOCUMENT_INCOMPLETE',
            'Complete the required document sections before approval',
          );
        if (brief.risks.some((r) => r.severity === 'red' && !r.mitigation.trim()))
          return fail('UNMITIGATED_RISK', 'Red risks require an explicit mitigation or assumption');
        if (brief.questions.some((q) => q.blocksEstimate))
          return fail('ESTIMATE_BLOCKED', 'Resolve blocking questions before approval');
        await tx.documentVersion.update({
          ...
          data: {
            ...
            approvedBy: userId,
            approvedAt: new Date(),
            deliveryKey: `scopeprofit-${saved.id}`,
          },
        });
  ...
  async deliver(projectId: string, version: number) {
    ...
    if (!project?.document) return fail('DOCUMENT_NOT_FOUND', 'Document not found', 404);
    const saved = await this.prisma.documentVersion.findUnique({
      where: { documentId_version: { documentId: project.document.id, version } },
    });
    if (!saved?.approvedAt) return fail('DOCUMENT_NOT_APPROVED', 'Document requires approval');
    if (saved.deliveredAt) return saved;
    if (!project.clientEmail)
      return fail('CLIENT_EMAIL_REQUIRED', 'Client must sign in before document delivery');
    const pdf = await this.download(projectId, version, 'pdf');
    const docx = await this.download(projectId, version, 'docx');
    await this.email.send(
      project.clientEmail,
      'Tu alcance aprobado — Scope-to-Profit',
      '<p>Adjuntamos el alcance aprobado en PDF y Word editable.</p>',
      [pdf, docx].map((file) => ({ name: file.name, content: file.buffer.toString('base64') })),
      saved.deliveryKey!,
    );
    return this.prisma.$transaction(async (tx) => {
      const changed = await tx.documentVersion.updateMany({
        where: { id: saved.id, deliveredAt: null },
