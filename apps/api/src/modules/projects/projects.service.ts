...
export class ProjectsService {
  ...
  async create(user: User, name: string, clientEmail?: string) {
    ...
    const project = await this.db.project.create({
      data: {
        ...
        document: { create: {} },
        links: { create: { tokenHash: hash(raw), encryptedToken: encrypt(raw) } },
      },
    });
    ...
    return { ...project, clientUrl: this.url(project.id, raw) };
  }
  url(id: string, raw: string) {
    return `${process.env.WEB_URL}/p/${id}?token=${raw}`;
  }
  async clientUrl(id: string) {
    ...
    return link ? this.url(id, decrypt(link.encryptedToken)) : null;
  }
  async list(user: User, limit = 30, cursor?: string) {
    const items = await this.db.project.findMany({
      where: user.role === 'professional' ? {} : { clientEmail: user.email ?? '' },
      orderBy: { id: 'asc' },
      take: limit + 1,
  ...
  async link(id: string, user: User, action: 'regenerate' | 'revoke' | 'expire', expiresAt?: Date) {
    this.auth.professional(user);
    await this.auth.project(user, id);
    const raw = token();
    await this.db.$transaction(async (tx) => {
      if (action === 'expire') {
        if (!expiresAt || expiresAt < new Date()) fail('INVALID_EXPIRATION');
        await tx.projectLink.updateMany({
        ...
        if (action === 'regenerate')
          await tx.projectLink.create({
            data: { projectId: id, tokenHash: hash(raw), encryptedToken: encrypt(raw) },
          });
      }
      await tx.auditEvent.create({
        data: { projectId: id, actorId: user.id, action: `link_${action}`, result: 'success' },
      });
    });
    return { clientUrl: action === 'regenerate' ? this.url(id, raw) : await this.clientUrl(id) };
  }
